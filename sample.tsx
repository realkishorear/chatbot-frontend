...
export default function Chatbot() {
    ...
    const sampleIndexRef = useRef<number | null>(null)
    ...
    useEffect(() => {
        ...
        const msg = JSON.parse(localStorage.getItem('messages') || 'null')
        if (msg)
            setMessages(msg)
    }, [])
    ...
    const fetchTreatments = async (branch_id: string, token: string) => {
        try {
            ...
            const res = await fetch(`http://localhost:8000/treatments/${organizationId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const fetchDoctors = async (branch_id: string, treatment_id: string, token: string) => {
        try {
            ...
            setIsTyping(true)
            const res = await fetch(`http://localhost:8000/doctors/${organizationId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
    ...
    const submitFinalForm = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:8000/appointment/${organizationId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${accessToken}`
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            while (true) {
                ...
                if (isFirstChunk) {
                    console.log("inside is First Chunk")
                    console.log(chunk.toLowerCase())
                    // First chunk is a boolean string: 'true' or 'false'
                    if(chunk.toLowerCase().trim() == "true")
                     {   console.log("chunk is appointment")
                        console.log(chunk.toLowerCase())
                        bookAppointmentFlag = true;
                     }
                    console.log(bookAppointmentFlag)
                    console.log(chunk.toLowerCase())

                    isFirstChunk = false;
    
                    if (bookAppointmentFlag) {
                        sendBotMessage('Can I get your Phone Number for verification?');
                        setFormStep(0);
                        return; // Exit early — form flow will handle rest
                    }
    
                    // Create new bot message to append streamed content into
                    setMessages(prev => {
                        const index = prev.length;
                        botMessageIndex = index;
                        sampleIndexRef.current = index;
                        return [
                            ...prev,
                            { role: 'bot', type: 'text', content: '', timestamp: now() }
                        ];
                    });
                    continue;
                }
                ...
                if (botMessageIndex !== null) {
                    setMessages(prev => {
                        const updated = [...prev];
                        if (!updated[botMessageIndex!]) return prev; // Safety
            ...
            sampleIndexRef.current = null; // Reset to allow next message to work properly
        }
    };
    ...
    const fetchPaitent = async () => {
        try {
            ...
            const res = await fetch(`http://localhost:8000/getFamilyMembers/${organizationId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${accessToken}`
    ...
    const createFamMemeber = async () => {
        ...
        try {
            setIsTyping(true)
            const res = await fetch(`http://localhost:8000/addFamilyMember/${organizationId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${accessToken}`
    ...
    return (
        <div className="w-screen h-screen flex items-center justify-center">
            <div className="w-full max-w-md h-full sm:h-[90vh] flex flex-col rounded-2xl shadow-md">
                ...
                <div
                    className="flex-1 h-0 overflow-hidden flex flex-col bg-[url('/chat_bg.jpg')] bg-cover bg-center"
                >
                    <div
                    ...
                    {formStep === 0 && (
                        <form
                            onSubmit={async e => {
                                ...
                                try {
                                    setIsTyping(true)
                                    const res = await fetch(`http://localhost:8000/generateotp/${organizationId}`, {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            number: phone
                    ...
                    {formStep === 1 && (
                        <form
                            onSubmit={async e => {
                                ...
                                try {
                                    setIsTyping(true)
                                    const res = await fetch(`http://localhost:8000/verifyotp/${organizationId}`, {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            number: formData.phoneNumber,
                    ...
                    {formStep === 2 && (
                        <form
                            onSubmit={async e => {
                                ...
                                try {
                                    setIsTyping(true)
                                    const res = await fetch(`http://localhost:8000/Register/${organizationId}`, {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                    ...
                    {formStep === 8 && (    
                        <form
                            ...
                            onSubmit={async e => {
                                ...
                                try {
                                    const res = await fetch(`http://localhost:8000/availability/${organizationId}`, {
                                        method: 'POST',
                                        headers: {
                                            'Content-Type': 'application/json',
                                            Authorization: `Bearer ${accessToken}`,
                                    ...
                                    await sendBotMessageWithDelay('Fill in the details to confirm appointment.')
                                    setFormStep(9)
                                } catch {
